...
function pasapalabra(question){
    ...
    function questionAnswer(){
            ...
            //Creamos dos variables para contar las preguntas acertadas y los fallos.
        ...
        //Una vez finalizado el juego, mostramos los aciertos y los fallos.
        //console.log("Tienes "+correctAnswers+" aciertos y "+incorrectAnswers+" fallos.");

        //Variable del total de preguntas contestadas
        var questionsanswered = correctAnswers+incorrectAnswers;
        console.log("Has contestado correctamente: "correctAnswers+" preguntas, y has fallado: "+incorrectAnswers);
    }

    //Preguntamos si quiere empezar el juego
    function askTurn(){
        var initiate = confirm("Si quieres empezar pulsa aceptar, sino pulsa cancelar");

        //Mostramos las instrucciones
        var initiate = confirm("Instrucciones: Irán apareciendo las descripciones de las palabras que empiecen por la letra en la que estés. Si te encallas en una letra, puedes guardarla para mas tarde escribiendo pasapalabra.");

        //Si el jugador quiere iniciar el juego
        if(initiate == true){

            //Metemos en una variable el numero de preguntas respuestas
            var question = questionAnswer();
            //Hacemos un bucle para que mientras no hayan respuesto las 27 preguntas, vuelva a preguntar las que hemos dicho pasapalabra.
            for(question;question!=27;question){
                var totalAnswers = questionAnswer();
    ...
    //Saludo
    console.log("Hola "+askName()+", estás iniciando el pasapalabra");

    //Iniciamos la funcion turno
    askTurn();
}
