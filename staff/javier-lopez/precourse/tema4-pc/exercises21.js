...
function sayItWithWords(number){
	...
	//Mostrar numeros del 10 al 19
		...
		var gettingNumbers = number.toString().split("");
		if(gettingNumbers[1]!=0 | gettingNumbers[2]!=0){
			console.log(units[gettingNumbers[0]]+" hundred and "+tens[gettingNumbers[1]]+units[gettingNumbers[2]]);
		}else{
			console.log(units[gettingNumbers[0]]+" hundred");
		}
