...
app.get('/files', (req, res) => {
  ...
  try {
    if (logic.isLoggedIn(session.username)) {
      ...
      res.render('file-get', {fileList: files, session, profileImage})
...
app.get('/register', (req, res) => {
  ...
  res.render('register-get', {session})
})
...
app.post('/register', (req, res) => {
  ...
  try {
    ...
    res.render('register-post', {username})
  } catch ({message}) {
...
app.get('/profile', (req, res) => {
  const {session} = req
  let userData, profileImage

  if (logic.isLoggedIn(session.username)) {
    try {
      userData = logic.getUserData(session.username)
      res.render('profile_get', {session, info: userData.info, profileImage: userData.profileImage})
    } catch {
      // ??
    }
  } else {
