...
describe('logic', () => {
  ...
  describe('update password', () => {
    ...
    beforeEach(() => {
      logic._users[username] = {password}
      newpwd = "something new"

    })

    it('should update password correctly', () => {
      logic.updatePassword(username, newpwd)
      expect(logic._users[username].password).to.equal(newpwd)
    })
    it('should throw error on empty new password', () => {
      expect(() => logic.updatePassword(username, "")).to.throw('invalid newpwd')
    })

    it('should throw error on invalid new password', () => {
      expect(() => logic.updatePassword(username, 123)).to.throw('invalid newpwd')
    })
    it('should throw error on invalid user name', () => {
      const username2 = username + Math.random()
      expect(() => logic.updatePassword(username2, newpwd)).to.throw(`user ${username2} does not exist`)
    })
  })
