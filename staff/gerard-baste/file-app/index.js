...
//         <ul>
//             ${files.map(file => `<li><a href="/download/${file}">${file}</a> <a href="/delete/${file}">[x]</a></li>`)}
//         </ul>
...
app.post('/register', (req, res) => {
    ...
    res.render('registerok', { username })
})
...
res.send(`<html>
     ...
     <body>
         ...
         <form action="/profile" method="post">
             <input type="text" name="username" disabled value="${session.username}">
             <input type="password" name="newPassword">
             <button type="submit">Update</button>
         </form>

         ${session.profileError ? `<h2 class="error">${session.profileError}</h2>` : ''}
     </body>
 </html>`)
...
app.post('/profile', (req, res) => {
    ...
    try {
        logic.updateProfile(username, newPassword)
