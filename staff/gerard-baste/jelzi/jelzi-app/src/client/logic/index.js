...
const logic = {
  ...
  _call(path, method, headers, body, expectedStatus) {
    ...
    return fetch(`${this.url}/${path}`, config).then(res => {
  ...
  addDish(email, titleDish, recipeId, sort, menuId, token) {
    return Promise.resolve()
      .then(() => {
        ...
        return this._call(
          `user/${email}/menus/${menuId}/dishes`,
          "post",
          {
            authorization: `bearer ${token}`,
            "content-type": "application/json"
          },
          JSON.stringify({ email, titleDish, recipeId, sort, menuId }),
          200
        );
      })
      .then(res => res.json());
  ...
  removeDish(email, menuId, id, token) {
    return Promise.resolve()
      .then(() => {
        ...
        return this._call(
          `user/${email}/menus/${menuId}/dishes/${id}`,
          "delete",
          { authorization: `bearer ${token}` },
          undefined,
          200
  ...
  searchRecipeById(email, menuId, token) {
    return Promise.resolve()
      .then(() => {
        ...
        return this._call(
          `user/${email}/search/menus/${menuId}`,
          "get",
          { authorization: `bearer ${token}` },
          undefined,
          200
