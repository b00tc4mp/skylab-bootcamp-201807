...
describe("logic", () => {
  ...
  (dishes = [
    {
      titleDish: "Breakfast",
      recipeId: "8c18f1819743bf2c06358d281db79743",
      sort: 1
    }
  ]),
    (titleDish = "Dinner"),
  ...
  after(() =>
    Promise.all([User.deleteMany()]).then(() => _connection.disconnect())
  );
  ...
  !true &&
    describe("validate fields", () => {
      it("should succeed on correct value", () => {
        expect(() =>
          logic._validateStringField("email", email).to.equal(email)
        );
        expect(() =>
          logic._validateStringField("username", username).to.equal(username)
        );
        expect(() =>
          logic._validateStringField("password", password).to.equal(password)
        );
      });

      it("should fail on undefined value", () => {
        expect(() => logic._validateStringField("name", undefined)).to.throw(
          `invalid name`
        );
      });

      it("should fail on empty value", () => {
        expect(() => logic._validateStringField("name", "")).to.throw(
          `invalid name`
        );
      });

      it("should fail on numeric value", () => {
        expect(() => logic._validateStringField("name", 123)).to.throw(
          `invalid name`
        );
      });
    });
  ...
  !true &&
    describe("register user", () => {
      ...
      it("should fail on trying to register with an undefined email", () =>
        logic
          .register(undefined, username, password, allergens)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`)));

      it("should fail on trying to register with an empty email", () =>
        logic
          .register("", username, password, allergens)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`)));

      it("should fail on trying to register with a numeric email", () =>
        logic
          .register(123, username, password, allergens)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`)));

      it("should fail on trying to register with an undefined username", () =>
        logic
          .register(email, undefined, password, allergens)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid username`)));

      it("should fail on trying to register with an empty username", () =>
        logic
          .register(email, "", password, allergens)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid username`)));

      it("should fail on trying to register with an undefined username", () =>
        logic
          .register(email, 123, password, allergens)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid username`)));

      it("should fail on trying to register with an undefined password", () =>
        logic
          .register(email, username, undefined, allergens)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid password`)));

      it("should fail on trying to register with an empty password", () =>
        logic
          .register(email, username, "", allergens)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid password`)));

      it("should fail on trying to register with a numeric password", () =>
        logic
          .register(email, username, 123, allergens)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid password`)));
  ...
  !true &&
    describe("authenticate user", () => {
      ...
      it("should login correctly", () =>
        logic.authenticate(email, password).then(res => {
      ...
      it("should fail on trying to login with a numeric password", () =>
        logic
          .authenticate(email, 123)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid password`)));
    });

  !true &&
    describe("retrieve allergens", () => {
      beforeEach(() =>
        User.create({
          email,
          username,
      ...
      it("should retrieve allergens correctly", () =>
        logic.retrieveProfileUser(email).then(res => {
      ...
      it("should fail on trying to retrieve allergens with a number email", () =>
        logic
          .retrieveProfileUser(123)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`)));
    });

  !true &&
    describe("update allergens", () => {
      ...
      it("should fail on trying to update allergens with an undefined email", () =>
        logic
          .updateAllergens(undefined, newAllergens)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`)));
  ...
  !true &&
    describe("save menu", () => {
      ...
      it("should fail on add undefined email", () =>
        logic
          .addMenu(undefined, title)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`)));
    });

  !true &&
    describe("save dish to menu", () => {
      beforeEach(() =>
        User.create({
          email,
          username,
      ...
      it("should fail on add undefined email", () => {
        return logic
          .addDish(undefined, titleDish, recipeId, sort, "3123123")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`));
      });
      ...
      it("should fail on add empty email", () => {
        return logic
          .addDish("", titleDish, recipeId, sort, "3123123")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`));
      });
      ...
      it("should fail on add number email", () => {
        return logic
          .addDish(123, titleDish, recipeId, sort, "3123123")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`));
      });
      ...
      it("should fail on add empty title dish", () => {
        return logic
          .addDish(email, "", recipeId, sort, "3123123")
          .catch(err => err)
          .then(({ message }) =>
      ...
      it("should fail on add number title dish", () => {
        return logic
          .addDish(email, 123, recipeId, sort, "3123123")
          .catch(err => err)
          .then(({ message }) =>
            expect(message).to.equal(`invalid title Dish`)
          );
      ...
      it("should fail on add undefined title dish", () => {
        return logic
          .addDish(email, undefined, recipeId, sort, "3123123")
          .catch(err => err)
          .then(({ message }) =>
            expect(message).to.equal(`invalid title Dish`)
          );
      ...
      it("should fail on add undefined recipe ID", () => {
        return logic
          .addDish(email, titleDish, undefined, sort, "3123123")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid recipe Id`));
      });

      it("should fail on add number recipe ID", () => {
        return logic
          .addDish(email, titleDish, 123, sort, "3123123")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid recipe Id`));
      });
      ...
      it("should fail on add empty recipe ID", () => {
        return logic
          .addDish(email, titleDish, "", sort, "3123123")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid recipe Id`));
      });
      ...
      it("should fail on add empty number sort", () => {
        return logic
          .addDish(email, titleDish, recipeId, "", "3123123")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid sort`));
      });

      it("should fail on add undefined number sort", () => {
        return logic
          .addDish(email, titleDish, recipeId, undefined, "3123123")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid sort`));
      });
      ...
      it("should create a dish without menu created ", () =>
        User.create({
          ...
          return logic
            .addDish(email, titleDish, recipeId, sort, undefined)
            .catch(err => err)
  ...
  !true &&
    describe("remove dish", () => {
          ...
          return user
            ...
            .then(user => {
              ...
              const dish2 = new Dish({
                titleDish: "Brunch",
                recipeId: "55450841e0c43087cb56d011602e443z",
                sort: 2
              });
            ...
            .then(user => {
              return logic
                .removeDish(email, menuId, dishId)
                .then(() => {
                  return User.findOne({
                    email
                  });
      ...
      it("should fail on delete existing menu with undefined email", () => {
        return logic
          .removeDish(undefined, "4342342", "34234234")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`));
      });
      ...
      it("should fail on delete existing menu with number email", () => {
        return logic
          .removeDish(34234, "4342342", "34234234")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`));
      });
      ...
      it("should fail on delete existing menu with undefined menuId", () => {
        return logic
          .removeDish(email, undefined, "34234234")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid Menu Id`));
      });

      it("should fail on delete existing menu with empty menuId", () => {
        return logic
          .removeDish(email, "", "34234234")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid Menu Id`));
      });
      ...
      it("should fail on delete existing menu with number menuId", () => {
        return logic
          .removeDish(email, 24324, "34234234")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid Menu Id`));
      });
      ...
      it("should fail on delete existing menu with undefined id", () => {
        return logic
          .removeDish(email, "24324", undefined)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid Dish Id`));
      });

      it("should fail on delete existing menu with empty id", () => {
        return logic
          .removeDish(email, "24324", "")
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid Dish Id`));
      });
  ...
  !true &&
    describe("List menu and dish", () => {
      ...
      it("should list all user menus", () => {
        return logic.listDishes(email, menuId).then(dishes => {
  ...
  !true &&
    describe("Search with allergens", () => {
      beforeEach(() =>
        User.create({
          email,
          username,
      ...
      it("Search", () => {
        return logic.searchRecipeAllergens("ramen", email).then(recipeData => {
          expect(recipeData.length).to.equal(20);
        });
      });
      it("should fail on search menu with undefined email", () => {
        return logic
          .searchRecipeAllergens("ramen", undefined)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`));
      });
      ...
      it("should fail on search menu with number email", () => {
        return logic
          .searchRecipeAllergens("ramen", 123)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`));
      });
      ...
      it("should fail on search menu with number query", () => {
        return logic
          .searchRecipeAllergens(123, email)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid query`));
      });
      ...
      it("should fail on search menu with undefined query", () => {
        return logic
          .searchRecipeAllergens(undefined, email)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid query`));
      });
    });
  ...
  !true &&
    describe("Basic search", () => {
      it("Basic search with query", () => {
        return logic.basicSearch("cake").then(recipeData => {
  ...
  !true &&
    describe("Search recipe by Id", () => {
      ...
      it("should search by recipe Id", () =>
        User.findOne({
          ...
          .then(user => {
            ...
            const dish2 = new Dish({
              titleDish: "Brunch",
              recipeId: "e22178517a25b7e55626de2f8c6f165a",
              sort: 2
            });
      ...
      it("should fail on search recipe by ID with undefined email", () => {
        return logic
          .searchRecipeById(undefined, menuId)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid email`));
      });
      ...
      it("should fail on search recipe by ID with undefined Menu ID", () => {
        return logic
          .searchRecipeById(email, undefined)
          .catch(err => err)
          .then(({ message }) => expect(message).to.equal(`invalid Menu ID`));
      });
    });
  ...
  !true &&
    describe("Basic search recipe ID", () => {
      it("Basic search with query", () => {
        return logic
          .basicSearchRecipeById("b66666d5c882ca199f43def8f1b8a03f")
          .then(recipesData => {
            expect(recipesData).to.exist;
          });
      });
